...
export const ChatPage: React.FC<ChatPageProps> = ({ onBack, appData, setData, profile }) => {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    if (!checkRateLimit()) {
      const errorMsg: Message = { role: 'assistant', content: 'Hata: Yapay zeka kullanım limitinize ulaştınız. Lütfen daha sonra tekrar deneyin. Limitlerinizi "Diğer" menüsünden kontrol edebilirsiniz.', id: `error-${window.crypto.randomUUID()}` };
      setCurrentMessages(prev => [...prev, errorMsg]);
      setSessions(prev => prev.map(s => s.id === currentSessionId ? { ...s, messages: [...s.messages, errorMsg], updatedAt: Date.now() } : s));
      return;
    }
    ...
    setSessions(prev => {
      ...
      if (exists) {
        return prev.map(s => s.id === currentSessionId ? { ...s, messages: newMessages, updatedAt: Date.now() } : s);
      } else {
    ...
    const encryptionKey = getRawKeyBase64();
    if (!encryptionKey) {
      const errorMsg: Message = { role: 'assistant', content: 'Hata: Şifreleme anahtarı bulunamadı.', id: `error-${window.crypto.randomUUID()}` };
      setCurrentMessages(prev => [...prev, errorMsg]);
      setSessions(prev => prev.map(s => s.id === currentSessionId ? { ...s, messages: [...s.messages, errorMsg], updatedAt: Date.now() } : s));
      setIsLoading(false);
      return;
    ...
    const sendRequest = async () => {
      try {
        ...
        if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
          ...
          const errorMsg: Message = { role: 'assistant', content: 'İnternet bağlantınız zayıf. Mesajınız kuyruğa alındı ve bağlantı düzeldiğinde otomatik olarak gönderilecek.', id: `queued-${chatId}` };
          setCurrentMessages(prev => [...prev, errorMsg]);
          setSessions(prev => prev.map(s => s.id === currentSessionId ? { ...s, messages: [...s.messages, errorMsg], updatedAt: Date.now() } : s));
        } else {
          const errorMsg: Message = { role: 'assistant', content: `Hata: ${error.message}`, id: `error-${chatId}` };
          setCurrentMessages(prev => [...prev, errorMsg]);
          setSessions(prev => prev.map(s => s.id === currentSessionId ? { ...s, messages: [...s.messages, errorMsg], updatedAt: Date.now() } : s));
        }
        setIsLoading(false);
  ...
  return (
    <div className="flex h-full w-full bg-[#F2F2F7] dark:bg-black relative overflow-hidden">
      ...
      `}>
        <div className="p-4 pt-[calc(1rem+env(safe-area-inset-top))] flex items-center justify-between border-b border-black/5 dark:border-white/5">
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 relative">
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 pt-[calc(0.75rem+env(safe-area-inset-top))] bg-white/70 dark:bg-[#1C1C1E]/70 backdrop-blur-xl border-b border-black/5 dark:border-white/5 sticky top-0 z-20">
          <div className="flex items-center gap-3">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 -ml-2 text-[#007AFF] hover:bg-black/5 dark:hover:bg-white/10 rounded-full transition-colors md:hidden">
              <Menu size={24} />
            </button>
            ...
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 -ml-2 text-[#007AFF] hover:bg-black/5 dark:hover:bg-white/10 rounded-full transition-colors hidden md:block">
              <Menu size={24} />
            </button>
            <div className="flex items-center gap-3">
        ...
        {/* Input Area */}
        <div className="px-4 py-3 pb-[calc(1rem+env(safe-area-inset-bottom))] md:pb-3 bg-white/70 dark:bg-[#1C1C1E]/70 backdrop-blur-xl border-t border-black/5 dark:border-white/5 sticky bottom-0 z-20">
